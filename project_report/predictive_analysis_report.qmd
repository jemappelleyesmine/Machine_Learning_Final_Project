...
We implemented a dual preprocessing strategy to meet the different requirements of the algorithms we choose as candidate models: a Linear Preprocessor for the Ridge baseline, and a Tree Preprocessor for Random Forest and Gradient Boosting models.

For numeric features (14 variables including age, working hours, remuneration, and community size), we applied median imputation in both preprocessors, and in the linear one, these numerical features were further standardized through StandardScaler to ensure proper convergence. For categorical features (29 variables including sex, family type, occupation codes, and employment characteristics), we applied most-frequent imputation followed by one-hot encoding in both both preprocessors. Seven numeric-coded categorical variables (such as employee count and employer type) were manually reassigned from the numeric feature list to the categorical feature list. 
...
The Gradient Boosting model was a 19.4% improvement over the Ridge Regression baseline (RMSE 0.341) and seemed to outperformed Random Forest by 0.0278 RMSE points, beyond the established threshold of 0.0061 based on cross-validation variance. The iterative tuning process we did gave diminishing returns, with the final iteration producing an improvement of 0.0021 RMSE, so less than the model's variance.

Therefore, Gradient Boosting was selected as the final model.

## Expected predictive performance on new data

The expected performances of our final model on new data are a root mean squared error of 0.2740, a mean absolute error of 0.1934, and an R squared of 0.7902. We obtained these estimates by evaluating the chosen Gradient Boosting model on a held-out test set of 10,010 observations (20% of the data) that was never used during model training or hyperparameter selection.
...
The model achieves a median absolute error of 0.1356. Ninety-five percent of prediction errors fall within the interval \[-0.5784, 0.5674\].

Analysis of the ten worst predictions reveals absolute errors ranging from 1.47 to 2.22, with the most severe case involving a true value of 0.0000 predicted as 2.2208. We examined the characteristics of these extreme errors. From our understanding, worst predictions predominantly occur for individuals from smaller communities (mean community size of 15,990 versus 40,998 overall), with lower current remuneration (18,637 versus 22,883) but much higher retirement pay (35,158 versus 18,773). Seven of the ten worst cases are male, five belong to family type "typem4-1," and two share the same socio-professional category (csp_2_2). This pattern could imply that the model has difficulty with individuals that exhibit unusual combinations of employment and retirement characteristics.

### Feature importance

![](diagnostics_importance.png)

We conducted permutation-based importance analysis on the full test set. It identifies age as the overwhelmingly dominant predictor, with ΔRMSE = 0.2499, nearly four times the importance of the second-ranked feature. When age information is randomly permuted, model performance degrades from RMSE = 0.2740 to approximately 0.5239, and R² decreases by 0.5575 (from 0.7901 to 0.2326). Community size, socio-professional category (OCCUPATION_42), and sport club participation (both count and binary membership indicator) form the second tier of importance with ΔRMSE values ranging from 0.062 to 0.068. Geographic features (region and department codes) rank sixth and seventh with ΔRMSE values around 0.054. Employment-related features like job type, economic sector, and remuneration have surprisingly low importance (ΔRMSE \< 0.01). The simplified job description codes (job_desc_N2) also show minimal importance (ΔRMSE = 0.0068).
